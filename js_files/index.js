var card= document.querySelector(".cards-container");
        ...
        fetch("./json_files/projects.json")
            ...
            .then((data)=>{
                data.map((item)=>{
                    card.innerHTML += '<div class="card" ><div class="card-front" data-aos="zoom-in-up"><p>'+item.p1+'</p><h2>'+item.h2+'</h2><button class="more">View More<span>&#129062;</span></button></div><div class="card-back"><p>'+item.p22+'</p><h2>'+item.h22+'</h2><p class="content">'+item.p222+' </p><button class="less">See Less<span>&#129062;</span></button></div></div>';
                })
})
...
var timeline= document.querySelector(".timelineul");
        ...
        fetch("./json_files/experience.json")
            .then((res)=>{return res.json()})
            .then((data)=>{
                data.map((item)=>{
